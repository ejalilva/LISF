...
!-------------------------------------------------------------------------
 ...
 CONTAINS
   SUBROUTINE L1RTB2ARFS_INVDIS(tim,tb_10h, tb_10v, tb_18h, tb_18v, tb_23h, tb_23v, tb_36h, tb_36v, tb_89h, tb_89v, &
          lat_l1r, lon_l1r, flag_l1r, nrows_l1rtb, ncols_l1rtb, land_water_frac &
          ref_lat, ref_lon, arfs_tim, arfs_flag_l1r, arfs_land_water_frac,&
   ...
   !SUBROUTINE L1BTB2ARFS_INVDIS(tim, tbvl1b_cor, tbhl1b_cor, tbvl1b, tbhl1b, surwat_v_l1b, surwat_h_l1b, &
     ...
     REAL*4,DIMENSION(nrows_l1btb,ncols_l1btb) :: tim, tb_10h, tb_10v, tb_18h, tb_18v, tb_23h, tb_23v, tb_36h, tb_36v, tb_89h, tb_89v, land_water_frac
     REAL*4,DIMENSION(nrows_l1btb,ncols_l1btb) :: lat_l1r, lon_l1r
     INTEGER*4,DIMENSION(nrows_l1btb,ncols_l1btb) :: flag_l1r
     INTEGER(4),DIMENSION(:,:),ALLOCATABLE :: zerodistflag
     REAL*8,DIMENSION(:),ALLOCATABLE :: ref_lat, ref_lon
     REAL*4,DIMENSION(2560,1920) :: arfs_tim, arfs_tb_10h, arfs_tb_10v, arfs_tb_18h, arfs_tb_18v, arfs_tb_23h, arfs_tb_23v, arfs_tb_36h, arfs_tb_36v, arfs_tb_89h, arfs_tb_89v, arfs_land_water_frac
     REAL*4,DIMENSION(2560,1920) :: arfs_wt_tim, arfs_wt_tb10v, arfs_wt_tb10h, arfs_wt_tb18v, arfs_wt_tb18h, arfs_wt_tb23v, arfs_wt_tb23h, arfs_wt_tb36v, arfs_wt_tb36h, arfs_wt_tb89v, arfs_wt_tb89h, arfs_wt_land_water_frac, arfs_wt_flag_l1r
     INTEGER*4,DIMENSION(2560,1920) :: arfs_samplenumv, arfs_samplenumh, arfs_flag_l1r
     ...
     arfs_flag_l1r=0 ! should I also initilize the quality flag
     ...
     DO ii = 1,ncols_l1rtb
           DO jj = 1,nrows_l1rtb
                 ...
                 ! start from here
                 IF (IBITS (tbv_qual_flag(jj,ii),qualitybit,1) == 0 .AND. IBITS (tbh_qual_flag(jj,jj),qualitybit,1) == 0) THEN !USE Snow and Precip here, I can add an if condition for soil moisture or the snow to wether filter or not
                    k=0
                       ...
                       DO cc =cmin,cmax !Lat direction
                          ...
                          IF (gcdist < search_radius) THEN !RESAMPLE ONLY WITHIN THE SEARCH RANGE
                             IF (gcdist < 0.0001D0) THEN !The TB is right on the grid center
                                ...
                                END IF
                                IF ((ABS (flag_l1r(jj,ii) - (-9999.0)).GT.1.0D-7)) THEN !DO IF NOT FILLVALUE(-9999)
                                   arfs_flag_l1r(rr,cc) = flag_l1r(jj,ii) ; arfs_wt_flag_l1r(rr,cc) = 1.0
                                END IF
                             ELSE
                                IF (zerodistflag (rr,cc).EQ.0) THEN
                                   ...
                                   END IF
                                   IF ((ABS (flag_l1r(jj,ii) - (-9999.0)).GT.1.0D-7)) THEN !DO IF NOT FILLVALUE(-9999)
                                      arfs_flag_l1r(rr,cc) = arfs_flag_l1r(rr,cc) + flag_l1r(jj,ii) / SNGL (gcdist*gcdist)
                                      arfs_wt_flag_l1r(rr,cc) = arfs_wt_flag_l1r(rr,cc) + 1.0 / SNGL (gcdist*gcdist)
                                   END IF
     ...
     WHERE(arfs_tb_23h.NE.0.0.AND.arfs_wt_tb23h.NE.0.0)
        arfs_tb_18v= arfs_tb_18v / arfs_wt_tb23h
     END WHERE
     WHERE(arfs_tb_18v.NE.0.0.AND.arfs_wt_tb18v.NE.0.0)
        arfs_tb_18v= arfs_tb_18v / arfs_wt_tb18v
     END WHERE
     ...
     WHERE(arfs_land_water_frac.NE.0.0.AND.arfs_wt_land_water_frac.NE.0.0)
        arfs_land_water_fracland_water_frac = arfs_land_water_frac / arfs_wt_land_water_frac
     END WHERE
     WHERE(arfs_flag_l1r.NE.0.0.AND.arfs_wt_flag_l1r.NE.0.0)
        arfs_flag_l1r = arfs_flag_l1r / arfs_wt_flag_l1r
     END WHERE
