...
!
! !INTERFACE:
subroutine readLIS_Teff_usaf_amsr(n, yyyymmdd, hh, Orbit, teff, rc)
! !USES:
  use LDT_coreMod
  use LDT_domainMod
  use LDT_logMod
...
!EOP
  integer :: c,r
  real    :: tsoil(LDT_rc%lnc(n),LDT_rc%lnr(n),4) ! E.J: last layer count changed form 4 to 1 since we just need the first layer
  character(255) :: fname
  logical :: file_exists
  integer :: rc1
  integer :: gid
  ...
  external :: create_LISsoilTemp_filename_usaf
  external :: read_LISsoilTemp_data_usaf
  ...
  nens = AMSReOPL%num_ens
  allocate(str_tind(LDT_rc%gnc(n) * LDT_rc%gnr(n)))
  if (AMSReOPL%ntiles_pergrid .ne. 1) then
     write(LDT_logunit,*) &
          '[ERR] Current AMSR_OPL code assumes ntiles_pergrid = 1'
     write(LDT_logunit,*) &
          '[ERR] Actual value is ', AMSReOPL%ntiles_pergrid
     call LDT_endrun()
  end if
  ...
  ntiles_pergrid = AMSReOPL%ntiles_pergrid

  call create_LISsoilTemp_filename_usaf(AMSReOPL%LISdir, &
       yyyymmdd, hh, fname)

  inquire(file=trim(fname), exist=file_exists)
  if (file_exists) then
     write(LDT_logunit,*) '[INFO] Reading ', trim(fname)
     call read_LISsoilTemp_data_usaf(n, AMSReOPL%num_tiles, str_tind, &
          ntiles_pergrid, nens, &
          fname, tsoil, rc1)
     if (rc1 .ne. 0) then
     ...
     do r = 1, LDT_rc%lnr(n)
        do c = 1, LDT_rc%lnc(n)
           ...
           ! For AMSR X-band (assumed to sense 0-2 cm), only the top layer  is used.
           if (tsoil(c,r,1) .gt. 273.15) then
                teff(c,r) = kk * tsoil(c,r,1)
           else
...
end subroutine readLIS_Teff_usaf_amsr

!BOP
!
! !ROUTINE: read_LISsoilTemp_data_usaf
! \label{read_LISsoilTemp_data_usaf}
!
! !INTERFACE:
subroutine read_LISsoilTemp_data_usaf(n, ntiles, str_tind, ntiles_pergrid, nens, &
     fname, tsoil, rc)
!
! !USES:
  use LDT_logMod
...
!EOP
  ...
  external :: calc_gridded_ensembleMean_1layer
  ...
  do k = 1, SoilTemp_profiles
     !call calc_gridded_ensembleMean_1layer(n, ntiles, str_tind, ntiles_pergrid, &
     !     nens, &
     !     SoilTemp_inst_tiles(:,k), &
     !     SoilTemp_inst_ensmean_1layer)

     call calc_gridded_lastEnsemble_1layer(n, ntiles, str_tind, ntiles_pergrid, &
          nens, &
          SoilTemp_inst_tiles(:,k), &
...
end subroutine read_LISsoilTemp_data_usaf

! Subroutine for calculating 2d gridded ensemble mean for a single soil layer,
! from tiled data.
subroutine calc_gridded_ensembleMean_1layer(n, ntiles, str_tind, ntiles_pergrid, &
     nens, gvar_tile, gvar)
...
end subroutine calc_gridded_ensembleMean_1layer

! Subroutine for extracting last ensemble member for a single soil layer,
! from tiled data.
 subroutine calc_gridded_lastEnsemble_1layer(n, ntiles, str_tind, ntiles_pergrid, &
            nens, gvar_tile, gvar)
 ...
 end subroutine calc_gridded_lastEnsemble_1layer

!BOP
! !ROUTINE: create_LISsoilTemp_filename_usaf
! \label{create_LISsoilTemp_filename_usaf}
!
! !INTERFACE:
subroutine create_LISsoilTemp_filename_usaf(LISdir, yyyymmdd, hh, filename)
 !USES:
...
end subroutine create_LISsoilTemp_filename_usaf
