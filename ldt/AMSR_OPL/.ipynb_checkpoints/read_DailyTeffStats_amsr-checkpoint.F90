...
! 
! !INTERFACE:
subroutine read_DailyTeffStats_amsr(doy)
! !USES:
  use LDT_logMod
  use LDT_amsr_oplMod
...
!EOP
  ...
  inquire(file=trim(AMSReOPL%dailystats_ref),exist=file_exists_ref)
  if(file_exists_ref) then
     write(LDT_logunit,*) '[INFO] Reading reference Teff daily mean and std dev: ',&
                          trim(AMSReOPL%dailystats_ref)
     call read_meanstddev(AMSReOPL%dailystats_ref,doy,&
                          AMSReOPL%ngrid,&
                          AMSReOPL%mu_6am_ref,AMSReOPL%mu_6pm_ref,&
                          AMSReOPL%sigma_6am_ref,AMSReOPL%sigma_6pm_ref,&
                          AMSReOPL%grid_col,AMSReOPL%grid_row)
     write(LDT_logunit,*) '[INFO] Finished reading ',trim(AMSReOPL%dailystats_ref)
  endif
    
  ! read lis Teff daily mean and std dev
  inquire(file=trim(AMSReOPL%dailystats_lis),exist=file_exists_lis)
  if(file_exists_lis) then
     write(LDT_logunit,*) '[INFO] Reading LIS Teff daily mean and std dev: ',&
                          trim(AMSReOPL%dailystats_lis)
     call read_meanstddev(AMSReOPL%dailystats_lis,doy,&
                          AMSReOPL%ngrid,&
                          AMSReOPL%mu_6am_lis,AMSReOPL%mu_6pm_lis,&
                          AMSReOPL%sigma_6am_lis,AMSReOPL%sigma_6pm_lis,&
                          AMSReOPL%grid_col,AMSReOPL%grid_row)
     write(LDT_logunit,*) '[INFO] Finished reading ',trim(AMSReOPL%dailystats_lis)
  endif

end subroutine read_DailyTeffStats_amsr
