...
!
! DESCRIPTION: Outputs AMSR based soil moisture retrieval in netCDF format.
!
!------------------------------------------------------------------------------
...
module LDT_AMSR_ARFSSM_netcdfMod
  ...
  ! Public routines
  public :: LDT_AMSR_ARFSSM_write_netcdf

contains

#if(defined USE_NETCDF3 || defined USE_NETCDF4)
  ! Subroutine for writing ARFS SM retrieval to netCDF
  subroutine LDT_AMSR_ARFSSM_write_netcdf(nc, nr, arfs_sm, retrieval_fname, &
       yyyymmdd, hhmmss)
    ...
    if (LDT_masterproc) then
       ...
       ! Create the output file
         ...
         case default
            write(LDT_logunit,*) &
                 '[ERR] Only latlon map projection supported for AMSR_OPL'
            call LDT_endrun()
         end select
...
#endif
    end subroutine LDT_AMSR_ARFSSM_write_netcdf
#else
  ! Dummy version
  subroutine LDT_AMSR_ARFSSM_write_netcdf(nc, nr, arfs_sm, retrieval_fname, &
       yyyymmdd, hhmmss)
...
end module LDT_AMSR_ARFSSM_netcdfMod
