...
!
! !INTERFACE:
subroutine readUSAFSI_amsr(n, yyyymmdd, hh, SnowDepth, rc)
...
!EOP
  ...
  external :: create_USAFsnowD_filename
  external :: read_usafSnowD_data
  ...
  do while (nn <= 24)
     ...
     call create_USAFsnowD_filename(AMSReOPL%LISsnowdir, &
          yyyymmdd_snow, hh_snow, fname)
     inquire(file=trim(fname), exist=file_exists)

     if (file_exists) then
        write(LDT_logunit,*) '[INFO] Reading snow depth from ', trim(fname)
        call read_usafSnowD_data(n, fname, SnowDepth, rc1)
        if (rc1 == 0) then
...
end subroutine readUSAFSI_amsr

!BOP
!
! !ROUTINE: read_usafSnowD_data
! \label{read_usafSnowD_data}
!
! !INTERFACE:
subroutine read_usafSnowD_data(n, fname, SnowDepth, rc)
!
! !USES:
  use LDT_logMod
  use LDT_coreMod
...
end subroutine read_usafSnowD_data

!BOP
! !ROUTINE: create_USAFsnowD_filename
! \label{create_USAFsnowD_filename}
!
! !INTERFACE:
subroutine create_USAFsnowD_filename(LISdir, yyyymmdd, hh, filename)
! !USES:
...
end subroutine create_USAFsnowD_filename
