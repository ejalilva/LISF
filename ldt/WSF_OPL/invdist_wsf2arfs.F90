...
CONTAINS
    ...
    SUBROUTINE WSF2ARFS_INVDIS(tim, tb_10h, tb_10v, tb_18h, tb_18v, &
        ...
        snow_in, precip_in, quality_flag, band_quality_flags, &
    ...
    integer*4, intent(in) :: quality_flag(nscans_in, nfovs_in)
    integer*4, intent(in) :: band_quality_flags(nscans_in, nfovs_in, 6)  ! NEW: per-band quality
    ...
    do ii = 1, nfovs_in
        do jj = 1, nscans_in
            ...
            band1_bad = (band_quality_flags(jj,ii,1) == 1)
            band2_bad = (band_quality_flags(jj,ii,2) == 1)
            band3_bad = (band_quality_flags(jj,ii,3) == 1)
            band4_bad = (band_quality_flags(jj,ii,4) == 1)
            band5_bad = (band_quality_flags(jj,ii,5) == 1)
            ...
            do rr = rmin, rmax
                do cc = cmin, cmax
                    ...
                    if (gcdist < search_radius) then
                        ...
                        ! Skip if cell already locked by exact match
                        if (cell_locked(rr,cc)) cycle
                        
                        if (gcdist < 0.0001D0) then
                            weight = 1.0D0
                            cell_locked(rr,cc) = .true.  ! Lock cell
                        else
                            weight = 1.0D0 / gcdist
                        endif
